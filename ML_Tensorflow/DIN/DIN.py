...
# from keras.layers import Layer, BatchNormalization, Dense, Input, PReLU, Embedding, Dropout
...
class Attention_layer(keras.layers.Layer):
    ...
    def __init__(self, att_hidden_units, activation='sigmoid'):
        ...
        self.att_dense = [keras.layers.Dense(unit, activation=activation) for unit in att_hidden_units]
        self.att_final_dense = keras.layers.Dense(1)
    ...
    def call(self, inputs):
        ...
        q = tf.tile(q, multiples=[1, k.shape[
            1]])  # (None, max_len*embedding)       # 沿着k.shap[1]的维度复制  毕竟每个历史行为都要和当前的商品计算相似关系
        ...
        outputs = tf.matmul(outputs,
                            v)  # 三维矩阵相乘， 相乘发生在后两维   (None, 1, max_len) * (None, max_len, embed_dim) = (None, 1, embed_dim)
...
class DIN(keras.Model):
    def __init__(self, feature_columns, behavior_feature_list, att_hidden_units=(80, 40), ffn_hidden_units=(80, 40),
        ...
        self.other_sparse_len = len(self.sparse_feature_columns) - len(
            behavior_feature_list)  # 这个other_sparse就是离散特征中去掉了能表示用户行为的特征列
        ...
        self.embed_sparse_layers = [keras.layers.Embedding(input_dim=feat['feat_num'],
                                                           input_length=1,
        ...
        self.embed_seq_layers = [keras.layers.Embedding(input_dim=feat['feat_num'],
                                                        input_length=1,
                                                        output_dim=feat['embed_dim'],
                                                        embeddings_initializer='random_uniform',
                                                        embeddings_regularizer=keras.regularizers.l2(
        ...
        self.bn = keras.layers.BatchNormalization(trainable=True)
    ...
    def call(self, inputs):
        ...
        mask = tf.cast(tf.not_equal(seq_inputs[:, :, 0], 0),
                       dtype=tf.float32)  # (None, maxlen)  类型转换函数， 把seq_input中不等于0的值转成float32
        ...
        for i in range(self.other_sparse_len):
            other_info = tf.concat([other_info, self.embed_sparse_layers[i](sparse_inputs[:, i])],
                                   axis=-1)  # (None, dense_len+other_sparse_len)

        # 下面把候选的商品和用户历史行为商品也各自的拼接起来
        seq_embed = tf.concat([self.embed_seq_layers[i](seq_inputs[:, :, i]) for i in range(self.behavior_num)],
                              axis=-1)  # [None, max_len, embed_dim]
        item_embed = tf.concat([self.embed_seq_layers[i](item_inputs[:, i]) for i in range(self.behavior_num)],
                               axis=-1)  # [None, embed_dim]
        ...
        if self.dense_len > 0 or self.other_sparse_len > 0:
            info_all = tf.concat([user_info, item_embed, other_info],
                                 axis=-1)  # (None, dense_len + other_sparse_len + embed_dim+embed_dim)
        else:
    ...
    def summary(self):
        dense_inputs = keras.layers.Input(shape=(self.dense_len), dtype=tf.float32)
        sparse_inputs = keras.layers.Input(shape=(self.other_sparse_len,), dtype=tf.int32)
        seq_inputs = keras.layers.Input(shape=(self.maxlen, self.behavior_num), dtype=tf.int32)
        item_inputs = keras.layers.Input(shape=(self.behavior_num,), dtype=tf.int32)
