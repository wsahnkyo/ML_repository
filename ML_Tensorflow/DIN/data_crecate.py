...
def create_amazon_electronic_dataset(file, embed_dim=8, maxlen=40):
	...
	print('=================Padding================')
	train_X = [np.array([0.] * len(train)), np.array([0]*len(train)), keras.preprocessing.sequence.pad_sequences(train['hist'], maxlen=maxlen), np.array(train['target_item'].tolist())]
	train_y = train['label'].values
	val_X = [np.array([0.] * len(val)), np.array([0]*len(val)), keras.preprocessing.sequence.pad_sequences(val['hist'], maxlen=maxlen), np.array(val['target_item'].tolist())]
	val_y = val['label'].values
	test_X = [np.array([0.] * len(test)), np.array([0]*len(test)), keras.preprocessing.sequence.pad_sequences(test['hist'], maxlen=maxlen), np.array(test['target_item'].tolist())]
