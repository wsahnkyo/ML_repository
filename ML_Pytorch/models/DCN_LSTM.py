...
class DCN_LSTM(nn.Module):
    def __init__(self, feature_columns, hidden_units, layer_num, dropout=0.):
        ...
        self.lstm = nn.LSTM(input_size=self.sparse_feature_cols[0]['embed_dim'],
                            hidden_size=self.sparse_feature_cols[0]['embed_dim'], num_layers=1)  # 传入参数
    ...
    def forward(self, x):
        ...
        x = torch.flatten(sparse_embeds, start_dim=1)
        ...
        # Deep Network
        (h0, c0) = (torch.zeros(1, sparse_embeds.shape[1], sparse_embeds.shape[2]),
                    torch.zeros(1, sparse_embeds.shape[1], sparse_embeds.shape[2]))  #
        lstm_out, (h,c) = self.lstm(sparse_embeds, (h0, c0))
        x2 = torch.flatten(lstm_out, start_dim=1)
        x2 = torch.cat([x2, dense_input], dim=1)
        deep_out = self.dnn_network(x2)
