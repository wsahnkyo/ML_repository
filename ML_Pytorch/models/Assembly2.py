...
class Assembly2(torch.nn.Module):
    def __init__(self, feature_columns, hidden_units, layer_num, dropout=0.):
        ...
        self.dense_embed_layers = NumEmbedding(len(self.dense_feature_cols), 1,
                                               self.sparse_feature_cols[0]['embed_dim'])
        self.cin = CIN(len(self.sparse_feature_cols) + len(self.dense_feature_cols), layer_num)  # layer_num是交叉网络的层数， hidden_units[0]表示输入的整体维度大小
    ...
    def forward(self, x):
        ...
        dense_embeds = self.dense_embed_layers(dense_input[..., None])
        # x = torch.cat(sparse_embeds, axis=-1)
        sparse_embeds = torch.stack(sparse_embeds, dim=1)
        x = torch.cat([sparse_embeds, dense_embeds], axis=1)
        # Wide
